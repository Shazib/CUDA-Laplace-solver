...
__global__ void deviceJacobi(const float *M_src, float *M_dest, int rows, int cols, size_t pitch)
{
	...
	if (row < 1000 && row > 0 && col < 1000 && col > 0) {
		...
		// Perform Jacobi
		rowDest[col] = (0.25) *  (rowSrc[col - 1] + rowSrc[col + 1] + rowDown[col] + rowUp[col]);
	}
}
...
/* HOST MEMORY INITIALISATION */
	...
	/* PERFORM COMPUTATION */
		...
		{			
			// Runs Last
			if (i % 2) {
				deviceJacobi <<<GRID, BLOCK>>>(B_d, A_d, ROWS, COLS, devPitchA);
				//cudaDeviceSynchronize();
			}
			// Runs first
			else {
				deviceJacobi <<<GRID, BLOCK>>>(A_d, B_d, ROWS, COLS, devPitchB);
				//cudaDeviceSynchronize();
			}
		}
		cudaEventRecord(stop);
